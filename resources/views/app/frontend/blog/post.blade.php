    ...
    <div class="w-full md:w-2/3 flex flex-col items-center mx-auto">
        ...
        <article class="flex flex-col shadow my-4">
            ...
            <div class="bg-white flex flex-col justify-start p-6">
                ...
                <p href="#" class="text-sm pb-8">
                    Oleh <a href="#" class="font-semibold hover:text-gray-800">{{$blog->admin()->first()->first_name." ".$blog->admin()->first()->last_name}}</a>, Dibuat tanggal {{date('d F Y', strtotime($blog->created_at))}}
                </p>
                {!!$blog->content!!}
            </div>
        </article>
        ...
        <div class="w-full flex pt-6">
            @if ($previous_blog->count()!=0)
                <a href="{{route('landing.single-blog', ['slug' => $previous_blog->first()->slug])}}" class="w-1/2 bg-white shadow hover:shadow-md text-left p-6">
            ...
            @if ($next_blog->count()!=0)
                <a href="{{route('landing.single-blog', ['slug' => $next_blog->first()->slug])}}" class="w-1/2 bg-white shadow hover:shadow-md text-right p-6">
